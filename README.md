...
`soul.nomad` is a combination of https://github.com/Nezreka/SoulSync/ and https://github.com/slskd/slskd - for music sharing and library management. 

# 2026 additional notes

I run three lab sites, but they all use `int.jeddi.org` for their domain, both with letsencrypt certs - and the Nomad clusters in both locations also have their own (via local DNS) roundrobin for `*.obs.int.jeddi.org` with valid SSL certs, managed by Traefik. This is what is served by the Nomad cluster at each site.

I'm using the env var `NOMAD_VAR_nomad_dc=DG` approach at my three sites (PY, DG, EN) so that more of my jobs can be run without modification at any location, and they work out where they are from that variable. This works well, and jobs that can really only be run at one site (due to compute requirements, say) just hard code the DC constraint.
